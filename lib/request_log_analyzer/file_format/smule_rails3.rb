module RequestLogAnalyzer::FileFormat
  ...
  class SmuleRails3 < Base
    ...
    line_definition :started do |line|
      line.header = true
      line.teaser = /Started /
      line.regexp = /app[0-9]+ [a-z\-]+\[([0-9]+)\]: Started ([A-Z]+) "([^"]+)" for (#{ip_address}) at (#{timestamp('%a %b %d %H:%M:%S %z %Y')}|#{timestamp('%Y-%m-%d %H:%M:%S %z')})/
    ...
    line_definition :processing do |line|
      ...
      line.regexp = /app[0-9]+ [a-z\-]+\[([0-9]+)\]: Processing by ([A-Za-z0-9\-:]+)\#(\w+) as(?:\s)([\w\/\*]*)?/
    ...
    line_definition :parameters do |line|
      ...
      line.regexp   = /app[0-9]+ [a-z\-]+\[([0-9]+)\]: Parameters:\s+(\{.*\})/
    ...
    line_definition :completed do |line|
      ...
      line.regexp = /app[0-9]+ [a-z\-]+\[([0-9]+)\]: Completed (\d+)? .*in (\d+(?:\.\d+)?)ms(?:[^\(]*\(Views: (\d+(?:\.\d+)?)ms .* ActiveRecord: (\d+(?:\.\d+)?)ms.*\))?/
    ...
    line_definition :failure do |line|
      line.footer = true
      line.regexp = /app[0-9]+ [a-z\-]+\[([0-9]+)\]: ((?:[A-Z]\w*[a-z]\w+\:\:)*[A-Z]\w*[a-z]\w+) \((.*)\)(?: on line #(\d+) of (.+))?\:\s*$/
